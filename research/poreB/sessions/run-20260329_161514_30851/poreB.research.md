...
The algorithm estimates the instantaneous shear strain history $\gamma(z, t)$ at user-specified depths within a one-dimensional soil stratum by a two-step procedure: it first reconstructs the depth-varying acceleration field $a(z, t)$ from the base excitation record via analytical frequency-domain transfer functions, then integrates the resulting acceleration field vertically to obtain the shear strain at each instant. The physical domain is a stratum of thickness $H_s$ with power-law shear modulus $G(z) = G_o(\eta/H_s)^m$, where $\eta = H_s - z$ denotes depth below the free surface, $G_o = \rho c_o^2$ is the base shear modulus, and $m \in [0, 2)$ is the profile exponent [KB:models.md]. Material damping with hysteretic ratio $\xi$ enters through the complex modulus $G^*(z) = G(z)(1 + 2i\xi)$, which replaces the real base wavenumber $k_o = \omega/c_o$ with its complex analogue $k_o^* = \omega/c_o^*$ where $c_o^* = c_o\sqrt{1 + 2i\xi}$ [KB:models.md].^[Confidence: HIGH, Rationale: The physical model, coordinate convention, power-law family, and complex-modulus damping formulation are stated explicitly in the KB. The two-stage structure follows directly from the kinematic integral relation in the KB and introduces no additional assumptions beyond what is documented there.]

For the uniform stratum ($m = 0$), the governing wave equation reduces to the constant-coefficient form and the zero-stress free-surface condition yields the closed-form acceleration transfer function [KB:models.md]:^[Confidence: HIGH, Rationale: The derivation and formula are given explicitly in the KB for the uniform profile, where the cosine solution and its resonant frequencies are derived from the boundary value problem.]

$$H_a(z,\,\omega) = \frac{\cos(k^*\,\eta)}{\cos(k^*\,H_s)}, \qquad k^* = \frac{\omega}{c_o^*}, \quad \eta = H_s - z$$

For the general power-law profile ($0 < m < 2$), the substitution $\zeta(\eta) = 2k_o H_s/(2-m)\cdot(\eta/H_s)^{(2-m)/2}$ transforms the governing ordinary differential equation into Bessel's equation of order $\nu = (1-m)/(2-m)$; excluding the singular $Y_\nu$ component by the regularity condition at the free surface and enforcing the base boundary condition gives the acceleration transfer function [KB:models.md]:^[Confidence: HIGH, Rationale: The Bessel substitution, the regularity argument excluding Ynu, and the resulting transfer function are derived and presented in full in the KB. The formula recovers the uniform case at m=0 via the trigonometric limit and the Gibson case at m=1 via the J0 identity, as confirmed in the KB.]

$$H_a(z,\,\omega) = \left(\frac{\eta}{H_s}\right)^{(1-m)/2} \frac{J_\nu\!\left[\zeta(\eta)\right]}{J_\nu\!\left[\zeta(H_s)\right]}, \qquad \nu = \frac{1-m}{2-m}$$

The time-domain acceleration at any depth is recovered by multiplying the base acceleration spectrum $A_g(\omega)$ by $H_a(z, \omega)$ and applying the inverse discrete Fourier transform. Both transfer functions are extended to include damping by evaluating $\zeta$ at the complex wavenumber $k_o^*$, while Bessel function values are computed at the real part of $\zeta$ -- a standard approximation that introduces errors of order $\xi^2$ in spectral amplitudes and is adequate for engineering damping ratios ($\xi \leq 0.2$) [KB:models.md].^[Confidence: MEDIUM, Rationale: The DFT product for acceleration recovery is a standard Fourier identity. The complex-wavenumber extension of the transfer functions is described in the KB. However, evaluating besselJ at Re(zeta) rather than the full complex argument is a practical approximation; the order-xi^2 error bound is a standard perturbation result but is not explicitly stated in the KB, representing a minor unverified claim.]

The shear strain $\gamma(\eta, t)$ is derived from $a(\eta, t)$ by integrating the equation of motion vertically from the free surface, where $\tau(H_s, t) = 0$, downward to depth $\eta$ [KB:models.md]:^[Confidence: HIGH, Rationale: The derivation and the exact integral formula are presented in the KB as an exact, model-independent consequence of the equation of motion, obtained by vertical integration and application of the zero-stress surface boundary condition.]

$$\gamma(\eta,\,t) = \frac{\rho}{G(\eta)} \int_0^{\eta} a(\eta',\,t)\,d\eta'$$

This expression is exact and model-independent: the shear stress $\tau = G\gamma$ at any level equals the total inertial force of the overlying soil column and does not depend on the specific form of $G(z)$ [KB:models.md]. At the free surface ($\eta = 0$), the integral vanishes and $\gamma = 0$, consistent with the zero-stress boundary condition. For power-law profiles with $m \geq 1/2$, the factor $G^{-1}(\eta) \propto (\eta/H_s)^{-m}$ causes the linearized strain to amplify toward the free surface, while $\tau$ remains bounded for all $m < 2$ [KB:models.md].^[Confidence: HIGH, Rationale: The model-independence of the shear stress (total inertia of overlying mass), the surface consequence (gamma=0 at eta=0), and the near-surface strain amplification for m>=1/2 are all explicitly stated in the KB and follow directly from the cited integral formula.]

The R implementation provides two public functions. `computeAccelField` executes Step 1: it zero-pads the input acceleration to the next power of two, computes the discrete Fourier transform, evaluates $H_a$ at each frequency for every point on a user-supplied depth grid, multiplies point-wise, and returns the real part of the inverse DFT as a numeric matrix with rows indexing time and columns indexing depth. `computeStrainHistory` executes the full pipeline: it builds a uniform grid in $\eta$, calls `computeAccelField` on that grid, accumulates the trapezoidal sum along the depth axis at each time step to form the cumulative integral $\mathcal{I}(\eta_k, t_i) = \int_0^{\eta_k} a(\eta', t_i)\,d\eta'$, linearly interpolates $\mathcal{I}$ to the requested evaluation depths, divides by $G(\eta)/\rho$, and returns a long-format `data.table` keyed by depth and time.^[Confidence: MEDIUM, Rationale: The function structure correctly implements the two-stage procedure derived from the KB. The trapezoidal integration is a standard numerical method not sourced from the KB; its accuracy depends on NIntGrid and the spatial smoothness of the acceleration field. The data.table output format is an implementation choice consistent with the stated constraints.]
