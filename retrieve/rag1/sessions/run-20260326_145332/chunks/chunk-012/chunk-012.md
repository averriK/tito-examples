...
$$k = -\frac{\ln[\lambda_{IM}(im_1)/\lambda_{IM}(im_2)]}{\ln(im_2/im_1)} = -\frac{\ln(0.000404/0.00211)}{\ln(0.29/0.18)} = 3.466$$

$$k_{im} = \lambda_{IM}(im_0) \cdot im_0^k = 0.00211 \times 0.18^{3.466} = 5.53 \times 10^{-6} \text{ year}^{-1}$$
...
$s = \bigl(\lambda_S(s)/k_{im} \cdot \exp(-(k/(2b^2))\,\beta_R^2)\bigr)^{-b/k} \cdot (1/a)$ … = 5.41 cm

(with $b = 0.5$, $k_{im} = 5.53 \times 10^{-6}$ year$^{-1}$, $\exp(3.466)$, $(0.2)$, $2k\,\beta_S\,2(0.5)\,b$)
...
Select ground motion intensity measures (IMs). Develop performance criteria in terms of EDPs. Select ground motion hazard. Select response model. Develop initial design. Compute response. Compare computed response curve with performance criteria. Iterate until performance criteria satisfied.

At the response level: Select response parameters (EDPs). Select response parameters (EDPs). Develop performance criteria in terms of EDPs. Select response model. Develop initial design. Compute response. Compare computed response with performance criteria. Iterate until performance criteria satisfied. Infer damage from computed response. Infer loss from inferred damage.

At the damage level: Select response parameters (EDPs). Select damage measures (DMs). Characterize damage limit states. Develop performance criteria in terms of DMs. Select response model. Select damage model. Develop initial design. Compute response. Compute damage. Compare computed damage with performance criteria. Iterate until performance criteria satisfied. Infer loss from computed damage.

At the loss level: Select response parameters (EDPs). Select damage measures (DMs). Select loss measures (DVs). Characterize loss levels. Develop performance criteria in terms of DVs. Select response model. Select damage model. Select loss model. Develop initial design. Compute response. Compute damage. Compute loss. Compare computed loss with performance criteria. Iterate until performance criteria satisfied. Loss directly computed.
...
$$\lambda(c) = \lambda_M^{\min} \sum_{i=1}^{N_{IM}} P[EDP > c \mid IM = im_i] \cdot P[IM = im_i] \tag{5.29}$$
...
$$\lambda_{LS} = \int_0^\infty \lambda(c) \cdot f_C(c) \, dc \tag{5.30}$$
...
$$\lambda_{EDP}(c) = k_{im} \left(\frac{c}{a}\right)^{-k/b} \exp\!\left(\frac{k}{2}\beta_R^2\right) \tag{5.31}$$
...
$$\lambda_{LS} = \int_0^\infty k_{im} \left(\frac{c}{a}\right)^{-k/b} \exp\!\left(\frac{k}{2}\beta_R^2\right) f_C(c) \, dc \tag{5.32}$$

Extracting $c$ from the first term and moving capacity-independent terms out of the integral,

$$\lambda_{LS} = \frac{k_{im}}{a^{k/b}} \exp\!\left(\frac{k}{2}\beta_R^2\right) \int_0^\infty c^{-k/b} f_C(c) \, dc \tag{5.33}$$
...
$$E[Y^\alpha] = \hat{Y}^\alpha \exp\!\left(\frac{1}{2}\alpha^2 \beta_Y^2\right) \tag{5.34}$$

Equation (5.33) becomes

$$\lambda_{LS} = k_{im} \left(\frac{\hat{C}}{a}\right)^{-k/b} \exp\!\left(\frac{k}{2b^2}\beta_R^2\right) \exp\!\left(\frac{k^2}{2b^2}\beta_C^2\right) \tag{5.35}$$

where $\hat{C}$ is the median capacity. This relationship can be simplified to

$$\lambda_{LS} = k_{im} \left(\frac{\hat{C}}{a}\right)^{-k/b} \exp\!\left(\frac{k}{2b^2}(\beta_R^2 + \beta_C^2)\right) \tag{5.36}$$
...
$$\lambda_{LS} = k_{im} \left(\frac{\hat{C}}{a}\right)^{-k/b} \exp\!\left(\frac{k}{2b^2}(\beta_R^2 + \beta_C^2)\right) = 0.0027 \text{ year}^{-1}$$

(with $k = 2.479$, $b = 0.5$, $\beta_R = 0.2$, $\beta_C = 0.3$, $\hat{C} = 20$ cm)

which gives a limit state return period of 370.4 years.

This closed-form solution, while conceptually quite clear and simple, must be recognized as being based on a local approximation to the hazard curve because actual ground motion hazard curves tend to be nonlinear (concave-down) in log-log-space. Its accuracy, therefore, is influenced by how the assumed power law IM hazard curve is fit to the actual IM hazard curve. The original approach of fitting a line tangent to the IM hazard curve at the IM level corresponding to the median capacity ($im = (\hat{C}/a)^{1/b}$) provides a conservative result since the tangent will always be above a concave-down hazard curve [@Jalayer2003]. At short return periods, where actual IM hazard curves tend to be more linear, the approach is more accurate than at long return periods. However, with the exception of highly vulnerable structures that can fail under weak shaking, earthquake engineers are usually more interested in performance at relatively long return periods. The error associated with the tangent approach becomes excessive with increasing IM hazard curve curvature and dispersion of capacity [@AslaniMiranda2005][@BradleyDhakal2008]. It can be substantially reduced, however, by selecting biased (toward shorter return periods) values of the points, $im_1$ and $im_2$, used to define a secant approximation (in Equation 5.22) to the IM hazard curve [@Vamvatsikos2014]. Letting $edp_C$ represent the EDP value corresponding to the median capacity $\hat{C}$, the IM values

$$im_1 = \left(\frac{edp_C}{a}\right)^{1/b} \exp\!\left(\frac{\beta_R + \beta_C}{b}\right) \tag{5.37a}$$

$$im_2 = \left(\frac{edp_C}{a}\right)^{1/b} \exp\!\left(-\frac{\beta_R + \beta_C}{b}\right) \tag{5.37b}$$
...
$$\lambda_{LS} = \phi\!\left(\lambda_{IM}(\hat{im}_C)\right) \exp\!\left(\frac{q \cdot k}{2b^2}(\beta_R^2 + \beta_C^2)\right) \tag{5.39}$$

where $\hat{C}$ is the median capacity, $\hat{im}_C$ is the intensity measure corresponding to the EDP value that produces the median capacity, i.e.,

$$\hat{im}_C = \left(\frac{\hat{C}}{a}\right)^{1/b}$$

$$\phi = \frac{k/b + \beta_R + \beta_C}{k/b}$$

$$q = k/b + \beta_R$$

and

$$\lambda_{IM}(\hat{im}_C) = \exp\!\left(-k_0 - k_1 \ln(\hat{im}_C) - k_2 (\ln \hat{im}_C)^2\right)$$
...
Pile Head Displacement Range (cm), EDP: < 4 / 4–10 / 10–30 / 30–100 / > 100

Damage State (DM) — Negligible: 0.95 / 0.05 / 0.00 / 0.00 / 0.00
Slight: 0.05 / 0.80 / 0.20 / 0.05 / 0.00
Moderate: 0.00 / 0.10 / 0.60 / 0.25 / 0.05
Severe: 0.00 / 0.05 / 0.15 / 0.55 / 0.10
Catastrophic: 0.00 / 0.00 / 0.05 / 0.15 / 0.85
...
$$\lambda_{EDP}(edp) = \int \int P[EDP > edp \mid IM_1 = im_1, IM_2 = im_2] \cdot MRD_{IM_1,IM_2}(im_1, im_2) \, d\,im_1 \, d\,im_2 \tag{5.41}$$

where $MRD_{IM_1,IM_2}(im_1, im_2)$ is the joint mean rate density (Equation 4.36), or equivalently,

$$\lambda_{EDP}(edp) = \int \int P[EDP > edp \mid IM_1 = im_1, IM_2 = im_2] \cdot f_{IM_2|IM_1}(im_2 \mid im_1) \cdot MRD_{IM_1}(im_1) \, d\,im_1 \, d\,im_2 \tag{5.42}$$

where the conditional IM distribution can be computed as

$$f_{IM_2|IM_1,R,M}(im_2 \mid im_1, r, m) = \int\!\!\int f_{IM_2|IM_1,R,M}(im_2 \mid im_1, r, m) \cdot f_{R,M|IM_1}(r, m \mid im_1) \, dr \, dm \tag{5.43}$$

and the quantity $f_{R,M|IM_1}(r, m \mid im_1)$ represents the joint distribution, i.e., the disaggregation, of moment magnitude $M$ and distance $R$, conditional upon $IM_1 = im_1$ (Section 4.4.3.5). The quantity $f_{IM_2|IM_1,R,M}(im_2 \mid im_1, r, m)$ depends on the GMMs for $IM_1$ and $IM_2$ and on the correlation between the two IMs. Discretizing $IM_1$ and $IM_2$ into $N_{IM_1}$ and $N_{IM_2}$ values, the mean annual rate of EDP exceedance can be approximated as

$$\lambda_{EDP}(edp) \approx \sum_{i=1}^{N_{IM_1}} \sum_{j=1}^{N_{IM_2}} P[EDP > edp \mid IM_1 = im_{1,i}, IM_2 = im_{2,j}] \cdot P[IM_2 = im_{2,j} \mid IM_1 = im_{1,i}] \cdot \Delta\lambda_{IM_1}(im_{1,i}) \tag{5.44}$$

where $P[EDP > edp \mid IM_1 = im_{1,i}, IM_2 = im_{2,j}]$ is obtained from probabilistic response analyses (Section 5.7.3), and the conditional probability of $im_{2,j}$ given $im_{1,i}$ is computed as

$$P[IM_2 = im_{2,j} \mid IM_1 = im_{1,i}] = \sum_{k=1}^{N_R} \sum_{n=1}^{N_M} P[IM_2 = im_{2,j} \mid IM_1 = im_{1,i}, R = r_k, M = m_n] \cdot P[R = r_k, M = m_n \mid IM_1 = im_{1,i}] \tag{5.45}$$

and

$$\Delta\lambda_{IM_1}(im_{1,j}) = \lambda_{IM_1}(im_{1,j-1}) - \lambda_{IM_1}(im_{1,j+1}) \tag{5.46}$$
...
$$P[IM_2 = im_{2,j} \mid IM_1 = im_{1,i}, R = r_k, M = m_n] = \Phi\!\left(\frac{\ln im_{2,j+1} - \mu_{\ln IM_2|IM_1=im_{1,i},r_k,m_n}}{\sigma_{\ln IM_2|IM_1=im_{1,i},r_k,m_n}}\right) - \Phi\!\left(\frac{\ln im_{2,j} - \mu_{\ln IM_2|IM_1=im_{1,i},r_k,m_n}}{\sigma_{\ln IM_2|IM_1=im_{1,i},r_k,m_n}}\right) \tag{5.47}$$

where

$$\mu_{\ln IM_2|IM_1=im_{1,i},r_k,m_n} = \mu_{\ln IM_2|r_k,m_n} + \rho_{\ln IM_1,\ln IM_2} \cdot \sigma_{\ln IM_1|r_k,m_n} \cdot \left(\ln im_{1,i} - \mu_{\ln IM_1|r_k,m_n}\right) / \sigma_{\ln IM_1|r_k,m_n} \tag{5.48}$$

$$\sigma_{\ln IM_2|IM_1=im_{1,i},r_k,m_n} = \sigma_{\ln IM_2|r_k,m_n} \sqrt{1 - \rho_{\ln IM_1,\ln IM_2}^2} \tag{5.49}$$
...
$$\lambda_{EDP_1 \text{ or } EDP_2}(edp_1, edp_2) = \int P[(EDP_1 > edp_1) \cup (EDP_2 > edp_2) \mid IM = im] \cdot |d\lambda_{IM}(im)| \tag{5.50}$$

which can be written, changing from the union to the intersection of events, as

$$\lambda_{EDP_1 \text{ or } EDP_2}(edp_1, edp_2) = \int P[1 - (EDP_1 \leq edp_1) \cap (EDP_2 \leq edp_2) \mid IM = im] \cdot |d\lambda_{IM}(im)| \tag{5.51}$$

or

$$\lambda_{EDP_1 \text{ or } EDP_2}(edp_1, edp_2) = \int \left[1 - F_{EDP_1,EDP_2|IM}(edp_1, edp_2 \mid im)\right] \cdot |d\lambda_{IM}(im)| \tag{5.52}$$

where $F_{EDP_1,EDP_2|IM}(edp_1, edp_2 \mid im)$ is the joint CDF of $EDP_1$ and $EDP_2$ conditional upon $IM$, and can be computed as

$$F_{EDP_1,EDP_2|IM}(edp_1, edp_2 \mid im) = \int_0^{edp_1}\!\!\int_0^{edp_2} f_{EDP_1,EDP_2|IM}(u_1, u_2 \mid im) \, du_1 \, du_2 \tag{5.53}$$
