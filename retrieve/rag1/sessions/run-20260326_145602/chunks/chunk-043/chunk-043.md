...
$$\rho \frac{\partial^2}{\partial t^2}\left(\frac{\partial \Phi}{\partial x} + \frac{\partial \Psi}{\partial z}\right) = (\lambda + 2\mu)\,\nabla^2\!\frac{\partial \Phi}{\partial x} + \mu\,\nabla^2\!\frac{\partial \Psi}{\partial z} \tag{C.38a}$$

$$\rho \frac{\partial^2}{\partial t^2}\left(\frac{\partial \Phi}{\partial z} - \frac{\partial \Psi}{\partial x}\right) = (\lambda + 2\mu)\,\nabla^2\!\frac{\partial \Phi}{\partial z} - \mu\,\nabla^2\!\frac{\partial \Psi}{\partial x} \tag{C.38b}$$

Solving Equations (C.38) simultaneously for $\partial\Phi/\partial t$ and $\partial\Psi/\partial t$ shows
...
$$\frac{d^2 F(z)}{dz^2} - \left(k_R^2 - \frac{\omega^2}{v_p^2}\right)F(z) = 0 \tag{C.41a}$$

$$\frac{d^2 G(z)}{dz^2} - \left(k_R^2 - \frac{\omega^2}{v_s^2}\right)G(z) = 0 \tag{C.41b}$$

which can be rearranged to give the second-order differential equations

$$\frac{d^2 F}{dz^2} - \left(k_R^2 - \frac{\omega^2}{v_p^2}\right)F = 0 \tag{C.42a}$$

$$\frac{d^2 G}{dz^2} - \left(k_R^2 - \frac{\omega^2}{v_s^2}\right)G = 0 \tag{C.42b}$$
...
$$\sigma_{zz} = \lambda\varepsilon + 2\mu\varepsilon_{zz} = \lambda\varepsilon + 2\mu\frac{dw}{dz} = 0 \tag{C.45a}$$

$$\sigma_{xz} = \mu\varepsilon_{xz} = \mu\left(\frac{dw}{du} + \frac{du}{dz}\right) = 0 \tag{C.45b}$$
...
$$\sigma_{zz}(z=0) = \left(\lambda + 2\mu\right)(-q^2)A_1 + 2\mu k_R^2 A_1 - 2\mu i k_R s A_2 = 0 \tag{C.46a}$$

$$\sigma_{xz}(z=0) = 2i k_R q A_1 + (s^2 + k_R^2) A_2 = 0 \tag{C.46b}$$

which can be rearranged to yield

$$\frac{(\lambda + 2\mu)q^2 - \lambda k_R^2}{\mu} \cdot A_1 = ik_R s\, A_2 \tag{C.47a}$$

$$iqk_R A_1 = -(s^2 + k_R^2) A_2 \tag{C.47b}$$
...
$$\mu^2(s^2 + k_R^2)^2 = (\lambda + 2\mu q^2)(\lambda k_R^2 - \lambda q^2) \tag{C.48}$$

which, upon introducing the definitions of $q$ and $s$ and factoring out a $G k_R$ term, yields

$$16\left(1 - \frac{\omega^2}{v_p^2 k_R^2}\right)\left(1 - \frac{\omega^2}{v_s^2 k_R^2}\right) - \left(2 - \frac{\omega^2}{v_s^2 k_R^2}\right)^2 = 0 \tag{C.49}$$
...
$$K_{Rs} = \frac{v_R}{v_p} \cdot \frac{v_p}{v_s} = \alpha_{Rs} \cdot \frac{v_p}{v_s}, \qquad \alpha_{Rs} = \frac{\mu}{\lambda + 2\mu} = \frac{(1-2\nu)}{2(1-\nu)} \tag{C.50b}$$

where $\alpha = \sqrt{\mu/(\lambda + 2\mu)} = \sqrt{(1-2\nu)/(2-2\nu)}$. Then Equation (C.49) can be rewritten as

$$\left(1 - K_{Rs}^2\right)\left(1 - \alpha K_{Rs}^2\right) = \frac{1}{16}\left(2 - K_{Rs}^2\right)^2\!\left(2 - \alpha K_{Rs}^2\right) \tag{C.51a}$$

which can be expanded and rearranged into the equation

$$K_{Rs}^6 - 8 K_{Rs}^4 + (24 - 16\alpha^2)K_{Rs}^2 - 16(1 - \alpha^2) = 0 \tag{C.51b}$$
...
$$u = -A_1 i k_R e^{-qz+i(\omega t - k_R x)} - A_2 s\,e^{-sz+i(\omega t - k_R x)} \tag{C.52a}$$

$$w = (-A_1 q\,e^{-qz+i(\omega t - k_R x)}) + A_2 i k_R e^{-sz+i(\omega t - k_R x)} \tag{C.52b}$$

FIGURE C.9 Variation of Rayleigh wave and body wave propagation velocities with Poisson's ratio.

Rearranging Equation (C.47b) as $A_2 = iqk_R A_1 / (s^2 + k_R)$ and substituting into Equations (C.52), gives

$$u = -A_1 i k_R \left(e^{-qz} - \frac{qs}{s^2 + k_R^2}\,e^{-sz}\right) e^{i(\omega t - k_R x)} \tag{C.53a}$$

$$w = A_1 k_R \left(-q\,e^{-qz} + \frac{k_R^2}{s^2 + k_R^2}\,e^{-sz}\right) e^{i(\omega t - k_R x)} \tag{C.53b}$$
...
$$G_1\left(\frac{\partial^2 v}{\partial x^2} + \frac{\partial^2 v}{\partial z^2}\right) = \rho_1 \frac{\partial^2 v}{\partial t^2} \quad \text{for } 0 \leq z \leq H$$

$$G_2\left(\frac{\partial^2 v}{\partial x^2} + \frac{\partial^2 v}{\partial z^2}\right) = \rho_2 \frac{\partial^2 v}{\partial t^2} \quad \text{for } z \geq H \tag{C.56}$$

The amplitude can be shown (Aki and Richards, 1980) [@AkiRichards1980] to vary with depth according to

$$V(z) = \begin{cases} A_1 e^{-\nu_1 z} + B_1 e^{\nu_1 z} & \text{for } 0 \leq z < H \\ A_2 e^{-\nu_2 z} + B_2 e^{\nu_2 z} & \text{for } z \geq H \end{cases} \tag{C.57a}$$

where the $A$ and $B$ coefficients describe the amplitudes of downgoing and upgoing waves, respectively, and

$$\nu_1 = \sqrt{k_L^2 - \omega^2/(G_1/\rho_1)}, \qquad \nu_2 = \sqrt{k_L^2 - \omega^2/(G_2/\rho_2)} \tag{C.57b}$$
...
$$V(z) = \begin{cases} A_1\left(e^{-\nu_1 z} + e^{\nu_1 z}\right) & \text{for } 0 \leq z < H \\ A_2\,e^{-\nu_2 z} & \text{for } z \geq H \end{cases} \tag{C.59}$$
...
$$v(x,z,t) = \begin{cases} A_1 \cos\!\left(\omega z \sqrt{\dfrac{1}{v_{s1}^2} - \dfrac{1}{v_L^2}}\right) e^{i(k_L x - \omega t)} & \text{for } 0 \leq z < H \\[6pt] A_1 \cos\!\left(\omega H \sqrt{\dfrac{1}{v_{s1}^2} - \dfrac{1}{v_L^2}}\right) \exp\!\left(-\omega(z-H)\sqrt{\dfrac{1}{v_L^2} - \dfrac{1}{v_{s2}^2}}\right) e^{i(k_L x - \omega t)} & \text{for } z \geq H \end{cases} \tag{C.63}$$
...
$$\tan\!\left(\omega H \sqrt{\frac{1}{v_{s1}^2} - \frac{1}{v_L^2}}\right) = \frac{G_2}{G_1} \cdot \frac{\sqrt{1/v_L^2 - 1/v_{s2}^2}}{\sqrt{1/v_{s1}^2 - 1/v_L^2}} \tag{C.64}$$
...
$$\sigma(x,t) = \sigma_I\,e^{i(\omega t - k_i x)} \tag{C.66a}$$
...
$$\sigma(x,t) = \sigma_T\,e^{i(\omega t - k_t x)} \tag{C.66b}$$

$$\sigma(x,t) = \sigma_R\,e^{i(\omega t + k_r x)} \tag{C.66c}$$

Assuming that the displacements associated with each of these waves are of the same harmonic form as the stresses that cause them, i.e.,
...
$$u(x,t) = A_I\,e^{i(\omega t - k_i x)} \tag{C.67a}$$

$$u(x,t) = A_R\,e^{i(\omega t + k_r x)} \tag{C.67b}$$
