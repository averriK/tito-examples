...
$$\left(\frac{\tilde{T}}{T}\right)^2 = 1 + \frac{k}{k_x} + \frac{kh^2}{k_{yy}} \tag{8.7}$$

which simplifies into the classical period lengthening expression [@VeletsosMeek1974]:

$$\frac{\tilde{T}}{T} = \sqrt{1 + \frac{k}{k_x} + \frac{kh^2}{k_{yy}}} \tag{8.8}$$
...
FIGURE 8.6 Plot of period lengthening ratio ($\tilde{T}/T$) and foundation damping $\xi_f$ versus structure-to-soil-stiffness ratio for square foundations ($L = B$) and varying structure aspect ratios, $h/B$. In this plot, $\nu = 0.33$, $B/L = 1.0$, hysteretic soil damping $\xi_s = 0$, mass ratio = 0.15, and exponent $n = 2$ (see Equations 8.9 and 8.10). (Figure modified from NIST, 2012.)
...
Givens et al. (2016) [@GivensEtAl2016] derived analytical models for foundation damping, while also explaining differences between foundation damping equations presented in prior studies [@VeletsosNair1975; @Bielak1975; @Roesset1980; @Wolf1985; @MaravasEtAl2014]. A suitable solution for the foundation damping ratio, $\xi_f$, for most applications is given by:

$$\xi_f = \left(\xi_x + \xi_s\right)\left(\frac{T_x^*}{\tilde{T}}\right)^n + \left(\xi_{yy} + \xi_s\right)\left(\frac{T_{yy}^*}{\tilde{T}}\right)^n \tag{8.10}$$
...
$$k_j^* = k_j(1 + 2i\xi_j) \tag{8.13}$$

where:

$$\xi_j = \frac{\omega c_j}{k_j} \tag{8.14}$$
...
Many impedance function solutions are available for rigid circular or rectangular foundations located on the surface of, or embedded within, a uniform, elastic, or visco-elastic half-space. In the case of a rigid rectangular foundation resting on the surface of a half-space with shear wave velocity $V_s$, Pais and Kausel (1988) [@PaisKausel1988], Gazetas (1991) [@Gazetas1991], and Mylonakis et al. (2006) [@MylonakisEtAl2006] summarize impedance solutions in the literature and present equations for the stiffness and damping terms in Equation (8.12). Solutions for the 2-D case, in which excitation is in the y-direction (parallel to the "short" dimension of the footing) are given by Gazetas and Roesset (1976) [@GazetasRoesset1976] and Jakub and Roesset (1977) [@JakubRoesset1977]. These solutions describe translational stiffness and damping along axes $x$, $y$, and $z$, and rotational stiffness and damping around those axes (denoted $xx$, $yy$, and $zz$) as shown in Figure 8.9a. For mode $j$,

FIGURE 8.9 Geometry and axis orientations for rectangular foundations (a) at the ground surface and (b) embedded to depth $H$. (Adapted from NIST, 2012 [@NIST2012].)

the stiffness, $k_j$, is represented as the product of the static (zero frequency) stiffness of the footing resting on the surface, $K_j$, a dynamic stiffness modifier, $\alpha_j$, and an embedment modifier, $\eta_j$:

$$k_j = K_j \alpha_j \eta_j \tag{8.16}$$

where $K_j = GB \cdot f(B/L, \nu)$, $\alpha_j = f(B/L, a_0)$, and $\eta_j = f(B/L, H/B, m)$; each of these functions are provided below in Tables 8.2 and 8.3a. In these functions, parameters $G$ and $\nu$ are the shear modulus and Poisson's ratio of the soil, $m = 1$ for translation of a rectangular foundation for the general 3D case ($m = 0$ for the 2D case corresponding to excitation in y-direction for long footing, i.e., $L \to \infty$), and $m = 3$ for rotation of a rectangular foundation for the general 3D case ($m = 2$ for the 2D case). The shear modulus, $G$, is evaluated using an equivalent-linear approach and as such should reflect the effects of modulus reduction with shear strain amplitude. Approximate adjustment factors for different site classes at different PGA levels are shown in Table 8.1. The maximum (or small strain) shear modulus, $G_{\max}$, is calculated from shear wave velocity as $G_{\max} = \rho_s V_s^2$ (Eq. 6.76). An average value of $V_s$ is generally computed across an effective profile depth, $z_p$, as described in Section 8.3.2.2. Dynamic stiffness modifiers, $\alpha_j$, are related to dimensionless frequency $a_0$:

$$a_0 = \frac{\omega B}{V_s} \tag{8.17}$$
...
Values of Shear Modulus Reduction and Hysteretic Soil Damping for Various NEHRP Site Classes (Section 7.3.3.1) and Shaking Amplitudes

Vs Reduction Factor $= \sqrt{G/G_{\max}}$; Value of $G/G_{\max}$; Value of $\xi_s$

PGA (g): ≤0.1 / 0.4 / ≥0.8 (columns repeated for each quantity)
...
Elastic Solutions for Static Stiffness of Rigid Footings at the Ground Surface and Embedded to depth $H$

Source: Solutions for rectangular foundations adapted from Pais and Kausel (1988) [@PaisKausel1988], as given by NIST (2012) [@NIST2012]. Solutions for 2D case apply for shaking in y-direction and are adapted from Gazetas and Roesset (1976) [@GazetasRoesset1976] and Jakub and Roesset (1977) [@JakubRoesset1977].

Degrees of freedom and equation references (surface solutions):

- Translation along y-axis (2D case): (T8.2.1-2)
- Rocking about x-axis (2D case): (T8.2.2-2)
- Translation along y-axis: (T8.2.3-2)
- Translation along x-axis: (T8.2.4-2)
- Translation along z-axis: (T8.2.5-2)
- Rocking about y-axis: (T8.2.6-2)
- Rocking about x-axis: (T8.2.7-2)
- Torsion about z-axis: (T8.2.8-2)

Surface Solution expressions (linearized from original table; refer to source document for full formula detail):

$K_{x,\text{sur}}$, $K_{y,\text{sur}}$, $K_{z,\text{sur}}$, $K_{xx,\text{sur}}$, $K_{yy,\text{sur}}$, $K_{zz,\text{sur}}$ — each of the form $K_j = GB \cdot f(B/L, \nu)$. Specific coefficients per degree of freedom:

- (T8.2.1-1): $K_{y,\text{sur}} = GB \cdot [\ldots]$
- (T8.2.2-1): $K_{x,\text{sur}} = GB \cdot [\ldots]$
- (T8.2.3-1): $K_{z,\text{sur}} = GB \cdot [\ldots]$
- (T8.2.4-1): $K_{yy,\text{sur}} = GB^3 \cdot [\ldots]$
- (T8.2.5-1): $K_{xx,\text{sur}} = GB^3 \cdot [\ldots]$
- (T8.2.6-1): $K_{zz,\text{sur}} = GB^3 \cdot [\ldots]$
- (T8.2.7-1): $K_{y,\text{sur,2D}} = GB \cdot [\ldots]$
- (T8.2.8-1): $K_{xx,\text{sur,2D}} = GB^3 \cdot [\ldots]$

Embedment modifier expressions (per degree of freedom): $\eta_x$, $\eta_y$, $\eta_z$, $\eta_{xx}$, $\eta_{yy}$, $\eta_{zz}$ — each of the form $\eta_j = f(H/B, L/B)$.

Notes: Axes should be oriented such that $L \geq B$. $G$ = shear modulus (reduced for large strain problems, e.g., Table 8.1).

### TABLE 8.3A

Dynamic Stiffness Modifiers and Radiation Damping Ratios for Rigid Surface Foundations [@PaisKausel1988; @NIST2012]

Degrees of freedom and equation references:

- Translation along x-axis: (T8.3.1-1), (T8.3.1-2)
- Translation along y-axis: (T8.3.2-1), (T8.3.2-2)
- Translation along z-axis: (T8.3.3-1), (T8.3.3-2)
- Rocking about y-axis: (T8.3.4-1), (T8.3.4-2)
- Rocking about x-axis: (T8.3.5-1), (T8.3.5-2)
- Torsion about z-axis: (T8.3.6-1), (T8.3.6-2)

Surface Stiffness Modifiers: $\alpha_x$, $\alpha_y$, $\alpha_z$, $\alpha_{xx}$, $\alpha_{yy}$, $\alpha_{zz}$ — each of the form $\alpha_j = f(a_0, B/L)$, with $\psi = \nu/(1-2\nu)$ and $a_0 = \omega B/V_s$.

Radiation Damping: $\xi_x$, $\xi_y$, $\xi_z$, $\xi_{xx}$, $\xi_{yy}$, $\xi_{zz}$ — each of the form $\xi_j = f(a_0, B/L) \cdot K_j/(GB)$, with $\psi \leq 2.5$.

Notes: Orient axes such that $L \geq B$. Hysteretic damping ($\xi_s$) is additive to radiation damping.

### TABLE 8.3B

Radiation Damping Ratios for Embedded Rigid Foundations [@PaisKausel1988; @NIST2012]

($\alpha_{\text{emb}} = \alpha_{\text{sur}}$; from Table 8.3a)

Degrees of freedom: Translation along x-axis, y-axis, z-axis; Rocking about y-axis, x-axis; Torsion about z-axis.

Radiation Damping: $\xi_{x,\text{emb}}$, $\xi_{y,\text{emb}}$, $\xi_{z,\text{emb}}$, $\xi_{xx,\text{emb}}$, $\xi_{yy,\text{emb}}$, $\xi_{zz,\text{emb}}$ — each of the form $\xi_{j,\text{emb}} = f(a_0, B/L, H/B) \cdot K_{j,\text{emb}}/(GB)$, with $\psi \leq 2.5$.

Notes: Hysteretic damping ($\xi_s$) is additive to radiation damping.

FIGURE 8.10 Plot of dynamic stiffness modifiers and damping ratios versus dimensionless frequency, for rectangular footings supported on the surface of a homogeneous half-space, with zero hysteretic damping, and $\nu = 0.33$: (a) x-direction (long dimension of footing); and (b) y-direction (short dimension). (Figure from NIST, 2012 [@NIST2012].)
...
For foundation stiffness calculations, it is best to start with the static stiffness of the foundations and then to make the dynamic adjustment per Equation (8.16). Static stiffnesses are computed using expressions of the form $K_j = GB \cdot f(B/L, \nu)$, with equations for specific vibration modes $j$ provided in Table 8.2. For the case of horizontal translation, the loading direction is taken in the x-direction (because the foundation is square, it could have equivalently been taken as the y-direction). From Table 8.2, using $G = \rho_s V_s^2$ and taking $\nu = 0.3$ (surficial materials are sandy):

$$K_{x,\text{sur}} = \frac{GB}{2-\nu}\left(6.8 \frac{L}{B} + 2.4\right)\bigg|_{B=2.03\text{ m}} = 7.8 \times 10^3 \text{ kN/m}$$

For the case of rotation caused by excitation in the x-direction, the rotation is about the yy-axis (Figure 8.9). From Table 8.2:

$$K_{yy,\text{sur}} = \frac{GB^3}{1-\nu}\left(3.73 + 0.27\frac{L}{B}\right)^{2.4}\bigg|_{B=2.03\text{ m}} = 3.4 \times 10^3 \text{ kN-m/rad}$$

The dynamic modifiers depend on dimensionless frequency. From Equation (8.17), $a_0 = 2\pi \times 2.03\text{ m} \times 6\text{ Hz} / 198\text{ m/s} = 0.39$.
