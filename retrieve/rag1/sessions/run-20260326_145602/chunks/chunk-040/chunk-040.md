...
$$A = M(\beta) = \frac{Q/k}{\sqrt{(1-\beta^2)^2 + (2\xi\beta)^2}} \tag{B.46}$$
...
$$Q(t) = a_0 + \sum_{n=1}^{\infty} \left(a_n \cos n\omega_n t + b_n \sin n\omega_n t\right) \tag{B.48}$$

where the Fourier coefficients are

$$a_0 = \frac{1}{T_f} \int_0^{T_f} Q(t)\, dt$$

$$a_n = \frac{2}{T_f} \int_0^{T_f} Q(t) \cos \omega_n t\, dt$$

$$b_n = \frac{2}{T_f} \int_0^{T_f} Q(t) \sin \omega_n t\, dt$$

and $\omega_n = n\pi / T_f$. Using the steady-state portion of Equation (B.44), the response to each sine term $n$ in the Fourier series is

$$u_{n,\sin}(t) = \frac{b_n}{k\sqrt{(1-\beta_n^2)^2 + (2\xi\beta_n)^2}} \left[(1-\beta_n^2)\sin\omega_n t - 2\xi\beta_n\cos\omega_n t\right] \tag{B.49a}$$

where $\beta_n = \omega_n T_f / (2\pi)$. In the same way, the steady-state response to each cosine term can be shown to be

$$u_{n,\cos}(t) = \frac{a_n}{k\sqrt{(1-\beta_n^2)^2 + (2\xi\beta_n)^2}} \left[(1-\beta_n^2)\cos\omega_n t + 2\xi\beta_n\sin\omega_n t\right] \tag{B.49b}$$
...
$$Q(t) = \sum_{n=-\infty}^{\infty} q_n^* e^{in\omega t} \tag{B.51}$$

The complex Fourier coefficients, $q_n^*$, can be determined directly from $Q(t)$ as

$$q_n^* = \frac{1}{T_f} \int_0^{T_f} Q(t)\, e^{-in\omega_n t}\, dt \tag{B.52}$$
...
$$m H(\omega_n) q_n^* e^{i\omega_n t}(-\omega_n^2) + ic\omega_n H(\omega_n) q_n^* e^{i\omega_n t} + kH(\omega_n) q_n^* e^{i\omega_n t} = q_n^* e^{i\omega_n t} \tag{B.55}$$

which can be rearranged to find the transfer function

$$H(\omega_n) = \frac{1}{k - m\omega_n^2 + ic\omega_n} = \frac{1}{k\left[(1-\beta_n^2) + 2i\xi\beta_n\right]} \tag{B.56a}$$
...
$$u(t) = \frac{Q_0}{k} + e^{-\xi\omega_0 t}\left(C_1 \sin\omega_d t + C_2 \cos\omega_d t\right) \tag{B.61}$$
...
$$u(0) = e^0\left(C_1 \sin 0 + C_2 \cos 0\right) + \frac{Q_0}{k} = C_2 + \frac{Q_0}{k} \tag{B.62a}$$

$$\dot{u}(0) = e^0\left[C_1\omega_d \cos 0 - C_2\omega_d \sin 0 - \xi\omega_0(C_1 \sin 0 + C_2 \cos 0)\right]$$

from which it can be shown that $C_2 = u_0 - Q_0/k$ and $C_1 = (\dot{u}_0 + \xi\omega_0 C_2)/\omega_d$, so that

$$u(t) = \frac{Q_0}{k} + e^{-\xi\omega_0 t}\left[\left(u_0 - \frac{Q_0}{k}\right)\cos\omega_d t + \frac{\dot{u}_0 + \xi\omega_0\left(u_0 - Q_0/k\right)}{\omega_d}\sin\omega_d t\right] \tag{B.63}$$
...
$$du(t) = \frac{Q(\tau)\, d\tau}{m\omega_d}\, e^{-\xi\omega_0(t-\tau)} \sin[\omega_d(t-\tau)] \tag{B.66}$$
...
$$u(t) = \sum_{i=1}^{n} \frac{Q(\tau_i)}{m\omega_d}\, e^{-\xi\omega_0(t-\tau_i)} \sin[\omega_d(t-\tau_i)]\, d\tau_i \tag{B.67}$$
...
$$W_D = \oint F\, du = \int_{t_0}^{t_0 + 2\pi/\omega} F\, \frac{du}{dt}\, dt = \pi c\omega u_0^2 \tag{B.70}$$
...
$$\beta_1 \approx 1 - \xi\sqrt{1-\xi^2} \tag{B.73a}$$

$$\beta_2 \approx 1 + \xi\sqrt{1-\xi^2} \tag{B.73b}$$
...
$$u(t) = \frac{Q_0}{k - m\omega^2 + ic\omega}\, e^{i\omega t} \tag{B.81}$$
...
$$u(t) = \frac{Q_0}{k^* - m\omega^2}\, e^{i\omega t} \tag{B.83}$$
