...
$\sigma_m$, $\sigma_m$, $\sigma_m$; $\sigma_m = 0.2$, and truncation limits set at $\pm 2\sigma_m$ from the mean.

The model is depicted in Figure 4.10(b) for $M_{\max} = 7.0$, $\sigma_m = 0.2$, and truncation limits set at $\pm 2\sigma_m$, i.e., $M_{\min} = 6.6$ and $M_{\max} = 7.4$.
...
( ) m M − β − − e min ( ) ( ) < < −∆ = m M M m M | F min max c

( ) M M M M − β − −∆ + − C e max min c (4.11a)

( ) M M M M − β − −∆ −∆ − −∆ β m M M e max min c ( ) ( ) M M M M − β − −∆ −∆ − −∆ β e max min c

( ) ( ) max c M −∆ < < = + m M m M | F max c max ( ) M M M M − β − −∆ + + − C C e max min c

where $C$ is a constant defined by:

( ) M M M M − β − −∆ −∆ β e max min c = ∆ M C c ( ) M M M − β − −∆ − e max min c

Differentiating $F_M(m)$ yields the PDF:

( ) m M − β − β e min ( ) ( ) < < −∆ = m M M m M | f min max c

( ) M M M M − β − −∆ + − C e max min c (4.11b)

( ) M M M M − β − −∆ −∆ β e max min c ( ) ( ) −∆ < < = m M M m M | f max c max

( ) M M M M − β − −∆ + − C e max min c

Other approaches for hybrid modeling of major active faults use a maximum magnitude model (truncated normal distribution) for the characteristic earthquake coupled with a truncated exponential model for smaller magnitudes (e.g., Field et al., 2009, 2014) [@FieldEtAl2009][@FieldEtAl2014].
...
where $\lambda_{M_{\min}}$ = the Poisson rate of occurrence of earthquakes with $m > M_{\min}$ and $M_0$ indicates seismic moment. The integral in Equation (4.13) represents the mean moment release for future earthquakes in the magnitude range of $M_{\min}$ to $M_{\max}$. The seismic moment, $M_0$, can be related to $m$ by re-arranging Equation (2.3) as
...
The Poisson rate term $\lambda_{M_{\min}}$ is evaluated by equating Equation (4.12) and (4.15).
...
2. Since the sources are shallow, the source-to-site distances are given below.

Source — Distance (km): 23.7, 25.0, 60.0

FIGURE E4.3 Positions of sources around site for Problem E4.3.

3. For this example, the peak acceleration is computed using the Campbell (1981) [@Campbell1981] GMM (Equation 3.25). This leads to the ground motions below:

Source — M — Distance (km) — PGA (g): 6.3 / 23.7 / 0.09; 6.7 / 25.0 / 0.12; 5.0 / 60.0 / 0.01

On this basis, Source 2 produces the controlling event. Note: Though currently out of date, the Campbell (1981) [@Campbell1981] relationship is used here because of its simplicity.

4. The hazard would be taken as that which results from a magnitude 6.7 earthquake occurring at a distance of 25 km, which produces PGA = 0.12g. Other IMs could be obtained from the GMMs described in Chapter 3.
...
$$P[IM > im] = \int_{\text{all}} P[IM > im \mid X = x] \, f_X(x) \, dx \tag{4.22}$$
...
$$P[IM > im \mid E] = \int \int P[IM > im \mid m, r] \, f_M(m) \, f_R(r) \, dm \, dr \tag{4.23}$$
